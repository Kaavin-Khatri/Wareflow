...
| Backup Storage | Automated DB backups & 14-day retention                           | Supabase Storage (`backups` bucket) + GitHub Artifacts | Global                     | Free                  | Daily 02:00 UTC Cron ([`.github/workflows/database-backup.yml`](file:///c:/Users/khatr/Documents/GitHub/Wareflow/.github/workflows/database-backup.yml)) |
...
| Decision                                              | Rationale                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ----------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Desktop Magnetic Cursor & Touch Fallback              | `CustomCursor` provides smooth spring-solved halo trailing and `1.6x` interactive target scaling exclusively on desktop viewports matching `(hover: hover) and (pointer: fine)`, automatically terminating on touch devices and honoring `prefers-reduced-motion: reduce`                                                                                                                                                                                 |
| Snappy Page-Load Choreography                         | Content entrance uses `<StaggerContainer>` with `staggerDelay=0.04` and `<FadeIn>` under 300ms total duration, creating an intentional, responsive reveal rather than sluggish staggered delays                                                                                                                                                                                                                                                           |
...
- **Supabase Free-Tier Backup Limitations & External Automated Mitigation**: Supabase's free tier does not provide Point-In-Time Recovery (PITR) and pauses idle projects after 7 days. To prevent data loss, WareFlow implements an automated daily GitHub Actions cron ([`.github/workflows/database-backup.yml`](file:///c:/Users/khatr/Documents/GitHub/Wareflow/.github/workflows/database-backup.yml)) executing `scripts/backup.py` at 02:00 UTC. Backups are compressed with GZIP, uploaded to Supabase Storage (`backups` bucket) and GitHub Artifacts with a 14-day rolling retention window. The disaster recovery restore procedure has been proven and verified against test data ([`docs/DISASTER_RECOVERY.md`](file:///c:/Users/khatr/Documents/GitHub/Wareflow/docs/DISASTER_RECOVERY.md)).
...
- **Google Places Indexing Lag**: Google's own indexing latency means a real-world opening may take Google days-to-weeks to list, which is the physical floor on 'new' detection speed. 'New' in WareFlow strictly indicates _first time seen in our discovery database_, not necessarily the moment doors opened.
...
| **P2**   | WhatsApp Inbound Conversational Bot   | Sales / Ordering     | Allow retailers to type _"Send 5 cases Parle-G and 2 bags Sugar"_ directly in WhatsApp to automatically create a draft Sales Order via LLM parser. |  Post-v1.2   |
