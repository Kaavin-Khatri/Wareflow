"""Weighted Moving Average demand forecasting strategy."""
...
class MovingAverageForecast(ForecastStrategy):
    """
    Weighted Moving Average forecasting strategy (default).

    Aggregates trailing outbound stock movements into weekly buckets and applies
    linearly increasing weights to more recent weeks.
    """
    ...
    def forecast(
        ...
        """Calculate weighted moving average demand across trailing weekly buckets."""
        ...
        weighted_sum = sum(w * q for w, q in zip(weights, weekly_buckets, strict=False))
        weighted_weekly = weighted_sum / sum(weights)
        daily_demand = round(weighted_weekly / 7.0, 4)
    ...
    def _aggregate_weekly_buckets(self, movements: list[Any], num_weeks: int = 4) -> list[float]:
        """Bucket movement quantities into 4 chronological weekly slots (oldest -> newest)."""
        ...
        for m in movements:
            ...
            if 0 <= week_idx < num_weeks:
                # Invert index so 0 is oldest, 3 is most recent week
    ...
    def _determine_trend(self, buckets: list[float]) -> str:
        """Infer directional trajectory comparing recent to older periods."""
        if len(buckets) < 2:
            return "stable"
        recent = buckets[-1]
    ...
    def _calculate_confidence(self, buckets: list[float], point_count: int) -> float:
        """Compute statistical confidence score based on consistency and data volume."""
        active_weeks = sum(1 for b in buckets if b > 0)
        base = active_weeks / float(len(buckets)) * 0.75
        volume_boost = min(point_count / 10.0, 1.0) * 0.20
        return round(min(base + volume_boost, 0.95), 3)
